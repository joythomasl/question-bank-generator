...
const questions = useMemo(
  ...
  function handleEditDifficulty(id, difficulty) {
    editQuestion(id, { difficulty, difficultyColor: DIFFICULTY_ROLE[difficulty] })
    setRefreshKey((k) => k + 1)
  }
  ...
  const verifiedPct = questions.length
    ? Math.round((questions.filter((q) => q.verified).length / questions.length) * 100)
    : 0
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium ${
            tab === 'manage' ? 'bg-surfaceRaised' : 'text-muted'
          }`}
        >
          Manage questions
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium ${
            tab === 'analytics' ? 'bg-surfaceRaised' : 'text-muted'
          }`}
        >
          Analytics
        </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleReset}
                className="text-xs text-muted border border-surfaceRaised rounded-lg px-3 py-1.5"
              >
                Reset overrides
              </button>
              <button
                onClick={handleExport}
                className="text-xs bg-catTwoPointers text-ink font-medium rounded-lg px-3 py-1.5"
              >
          ...
          <div className="flex flex-col gap-2">
            {questions.map((q) => (
              <div
                key={q.id}
                className="bg-surface border border-surfaceRaised rounded-lg px-4 py-3 flex items-center gap-4"
              >
                ...
                <button
                  ...
                  className="text-xs text-danger border border-surfaceRaised rounded-lg px-2 py-1"
                >
                  Remove
                </button>
              </div>
          ...
          <p className="text-xs text-muted mt-6">
            Edits and removals here are stored in this browser only. Use export to bake the
            curated set into the questions.json shipped with the deployed site.
          </p>
        </div>
      )}
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(categoryCounts).map(([cat, count]) => (
                <div key={cat} className="flex items-center gap-3">
                  <span className="text-xs w-40 text-muted">{cat}</span>
                  <div className="flex-1 bg-ink rounded-full h-2">
                    <div
                      className="bg-catDp h-2 rounded-full"
                      style={{ width: `${questions.length ? (count / questions.length) * 100 : 0}%` }}
                    />
                  </div>
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(difficultyCounts).map(([diff, count]) => (
                <div key={diff} className="flex items-center gap-3">
                  <span className="text-xs w-40 text-muted">{diff}</span>
                  <div className="flex-1 bg-ink rounded-full h-2">
                    <div
                      className="bg-warn h-2 rounded-full"
                      style={{ width: `${questions.length ? (count / questions.length) * 100 : 0}%` }}
                    />
                  </div>
                  <span className="text-xs w-8 text-right">{count}</span>
